...
Up until now we have used the command line to run programs on the local fule system. However, it is also possible to connect to another system using the command line. SSH is an encrypted connection protocol, which allows logging in to another computer using the command line. An encrypted connection means that infromation is not shared in a human readable plain text form. The goal is to make sure that only the intended recipients can decrypt the message and read its contents.

In a Unix environment an SSH connection can be established with the command `ssh`. Its basic functionality is the following: `ssh user@server.address` connects to a server at `server.address` and tries to login as `user`. The user is asked their password, and if authentication is successfull, a connection is opened. The working directory is changed to the remote server on the command line, and one can run commands as in the local environment, except the commands are run in the remote system where one just connected.
...
Authentication with an SSH key pair is possible, when the public key is added to the server where one wishes to be authenticated. Then one can log in to the system, if one knows the private key corresponding to the public key. The puvlic key is added to the path `~/.ssh/authorized_keys`, that is to the folder `.ssh` in the user's home folder, to the file [authorized_keys](https://www.ssh.com/ssh/authorized_keys/). If you are using multiple keys, each one is added to a new line in the file. You'll probably have to create the folder and file yourself on the university's server.
...
The copying of the public file can also be delegated to the command [ssh-copy-id](https://www.ssh.com/ssh/copy-id).
...
In order not to rewrite the passoword each time you want to log in, you might want to add the password to an ssh agent. Then the password has to only be written once you've logged out of the system. This can happen using two commands.
...
Create an SSH key pair on your computer. Then enable logging in to the department's servers with the key when establishing an SSH connection. Then test out the key pair. 

If possible, do this using other than the department's computers. If you don't have access to the department's servers, i.e. you do not have a university account, you can mark the exercise don after generating the SSH key pair.

<div class="note">
If you do the exercise using a department's computer, the connection is likely to act weirdly, because the connection is established to the same system. The password of the SSH key  might not be asked even if you had set it correctly.
</div>

<div class="note">
If you defined the location of the key pair yourseld, and the connection still asks for your university password, specify the path to the private key:
<div>
...
<div class="exercise">
<h3>Exercise 12: Wget and HTML code {% include points.html text="10%" %}</h3>

<ol>
<li>Download a site of your liking to your computer using <code>wget</code></li>
<li>Open the <code>.html</code> extensioned file using a graphical browser, esim Firefoxilla tai Chromella (try clicking it in the graphical file browser). The site will most likely look different from what you're used to. </li>
...
<p>
The goal of this exercise is to count how many lines of the Wikipedia article about Linus Torvalds contain the word "Linux".
</p>
...
In Unix-like systems, the ability to read, edit and execute files and folders is controlled with permissions. This is especially useful in a system which is used by several users, such as the the department's systems.

Reading permission allows the user to _read_ the contents of a file. A user with the permission to _write_ can edit a file. The permission to _execute_ means that if the file is a program for example, the user can run it. Execute permissions are also required in order to move inside a folder with `cd`. If a user doesn't have the necessary persmission, usually the error `Permission denied` is printed out.

Each file and folder have their own permissions. In addition, they all have an owner. The user who created the file is the owner by default, but ownership can also be transferred.
...
Each file and folder in the directory has its own line. The first column in the output, containing mixtures of letters and dashes describes the permissions of the file. Here the first letter is either `-` or `d`, which tells if the resource is a file `-` or a directory `d`.
...
The next three symbols reveal the permissions of the group og the file. A group is a way of bundling users together. For example all the students of the Department of computer science could be in the same group. A file is assigned to a group, and the group can have certain permissions. These permissions apply to all members of the group.
...
If the recipent of permissions is not defined, the permissions are given to all user groups (owner, group and others).
...
<div class="exercise">
<h3>Exercise 14: Practicing permissions {% include points.html text="10%" %}</h3>

Create a file called <code>permission.txt</code> and practice giving and removing permissions. Define the file's permissions to be each of the following:
