...
By also changing the value of `position`, we have managed to move the element as we wished. The value `relative` doesn't change the position of the element in the page structure, but allows moving the element with regards to it's original location. 
