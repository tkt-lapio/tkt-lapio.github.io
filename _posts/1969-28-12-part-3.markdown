...
The previous parts of the course covered doing several tasks on the command line, and how version control can be used for storing and sharing code. In this part we will use those skills to create simple, static websites. We will start by discussing what static websites actually are, and what they consist of.
...
<li>Add a file called <code>index.html</code> to the project, and copy the code from <a href="https://github.com/rovaniemi/lapio-tehtavamateriaalia/blob/master/vk3-ex1/index.html">here</a> inside. You don't need to understand the contents of the file yet.</li>
...
HTML tags are keywords hidden inside the site, but not shown to the end user by browser. Nevertheless, they define how the browser styles and shows the contents of the document you are viewing. There are countless different HTML tags, each serving to create specific kinds of elements. We'll start with the elements used to edit text.
...
Next we'll get to know the two probably most commonly used elements in HTML: `<div></div>` and `<span></span>`. The `span` tag creates an inline element, and the `div` tag creates a block level element. In practice, both of them are empty shells, meaning that they do not impose a particular style or outlook on the elements they create. They are in fact usually used for wrapping groups of elements together. This facilitates adding CSS to a large group of elements, an particularly customizing the layot of page. Let's focus on the latter next.
...
![An example of height and width](/assets/html-example-12.png)

We notice that the CSS above created a red rectangle around the `p` element. This is due to the fact that we explicitly defined the size, i.e. the widht and the height of the `div` element, which contains the element containing the actual text you see on the website.
...
There are several different ways and units used to define the value of the margin, padding and other CSS properties. We'll go through all these different possible ways by using the margin property as an example. The same rules apply to padding, for example, which is used in the same way.
...
![Example HTML page with two elements](/assets/html-example-13.png)

Let's start by getting to know the property whcih allows moving elements on a web page. 

We'll strat by giving the `.second-box-container` selector the property `left: 250px;`. This should move the middle HTML element 250px away from the left side of the page:
...
![Example HTML page with position relative](/assets/html-example-14.png)
...
![Example HTML page with position absolute](/assets/html-example-15.png)
...
<li>Edit the HTML and CSS code in order to make the webiste look like the following:</li>
<img src="/assets/html-ex-9.png" alt="Adjacent elements"/>
<li>When you think you are done, create a commit with the message <code>Part 3, ninth exercise</code>, and push it to GitHub.</li>
...
5. `vw` refers to the width of the screen. `1vw` means one percent of the screens width. 
...
`line-height` can be used to change the distance between lines. However, the property will only have an effect on text. The values should be given as pixels or percents.

`font-family` defines the font of the element.

`font-size` defines the size of the text. The values should be given in `px` or `em`.

`font-style` allows styling the font further, by for example making it bold or cursive. Nevertheless, bolded text should always be created using `<strong></strong>` or `<b></b>` elements, and cursive with the elements `<em></em>`. This is because the screen readers, that for example blind people use, only use the HTML of the page, and thus wouldn't be able to make the distinction between normal text and text bolded with CSS.
...
<div class="exercise">
<h3>Exercise 10: Practicing CSS properties {% include points.html text="5%" %}</h3>
<ol>
...
<li>When you are done in your opinion, create a commit with the message <code>Part 3, tenth exercise</code> and push it to GitHub.</li>
</ol>
</div>
...
![HTML with no CSS](/assets/html-example-7.png)
...
Let's start by centering the text on teh page. This can be done by adding the `margin` property and the `max-width` properties to the `body` tag.
...
![Centered HTML page](/assets/html-example-8.png)
...
![HTML page with less contrast](/assets/html-example-10.png)

Much better. Now let's make the code on the page stand out. This can be done by using separate elements, built with the tag `code`, for the code blocks, and giving them the following styles:
...
![HTML page with separate code blocks](/assets/html-example-11.png)
...
<ol>
<li>Make the example website more readable accoring to the instructions above. // TODO: what to use as text?</a></li>
...
There are several different places one can use as a platform for publishing a static website. A common way is to buy or rent a server, and use it to serve documents to the world. On this course, we'll actually use GitHub to publish a static website. 
...
Student's who are currently enrolled at the University of Helsinki are given some server space to run their own static wevsites. The website can be published by adding the required documents to the user's CS home folde,r in the path <code>/cs/home/username/public_html/</code>. The website will be published to the following address: <code>http://www.cs.helsinki.fi/u/käyttäjätunnus/</code>

In practice, here are instructions on publishing the website:
...
<li>Publishing the page will require giving some sites some additional permissions:</li>
<ol>
<li>public_html: All users should have read and execute permissions.</li>
<li>cs home folder (cs/home/username): execute permissions for everyone.</li>
<li>The site should now be live.</li>
</ol>
</ol>
...
<li>Take advantage of the things you have learned during this course. It might be a good idea to search for inspiration with for example the key owrds <code>static website resumes</code>. If you don't know how to do something, just try and Google it.</li>
...
This is the end of the last part of the course. You should now have a basin understanding about the command line, Git and HTML/CSS. This knowledge will certaily deepen during your studies, as these are all important tools for a computer scientist.
...
Take a break, and congratulate yourself for finishing reading the material. Then go back to the learning goals you can find from the beginning of this part, and see which subjects seem distant to you. If you have trouble remembering something, check the material or the Internet for answers. Finally, make sure to read the exam instructions before starting it. Also make sure to mark all the exercises you have done on Moodle, so we can use them in grading, and that you have enrolled on the course in Weboodi. 

<div class="warning">
Give anonymous feedback <a href="https://weboodi.helsinki.fi/hy/etusivu.html?Kieli=6"> in Weboodi</a> kohdasta > Tools > My Feedback.
</div>
