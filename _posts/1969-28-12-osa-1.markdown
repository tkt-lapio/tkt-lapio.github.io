...
{% capture note_auto_1 %}
Komentorivi on englanniksi <i>command line</i> tai <i>terminal</i>. Siihen viitataan joskus myös sanalla <i>shell</i> tai <i>console</i>.
{% endcapture %}

{% include message.html type="info" title="Rakkaalla lapsella on monta nimeä" text=note_auto_1 %}
...
{% include message.html type="info" title="Vakiintunut käytäntö" text=note_dollar_sign %}
...
{% include message.html type="info" title="Komentojen lyh. nimet" text=note_abbrev_cmd %}
...
{% include message.html type="info" title="Kotihakemisto Windowsilla" text=note_wsl_dir %}
...
{% include message.html type="info" title="Kopioiminen terminaalista" text=note_term_copy %}
...
Oletuksena uuden komentorivin avaaminen avaa sen nimenomaan kotihakemistoon, joten tarvitsemme tavan vaihtaa aktiivista kansiota jotta voimme vierailla muissa kansioissa ja tarkastella niiden sisältöjä. Tarvittava komento on `cd` (**c**hange **d**irectory). `cd`-komennolle annetaan parametrina se kansio, johon halutaan siirtyä. Mikäli et anna `cd`:lle argumentteja, se siirtää sinut kotihakemistoosi.
...
{% include exercise.html points="5%" title="Tehtävä 1: Graafinen vs. tekstikäyttöliittymä" content=exercise_1 %}
...
{% include exercise.html points="5%" title="Tehtävä 2: Kansion luominen" content=exercise_2 %}
...
Tehtyäsi `esimerkki.txt`-tiedoston navigoi komentorivillä `lapio`-kansioon, ja avaa se komentamalla `nano esimerkki.txt`. Nano on tekstieditori, jossa on tekstikäyttöliittymä, eli jota käytetään suoraan komentoriviltä. Tämä tarkoittaa, että ohjelmaa hallinnoidaan tietyillä näppäinyhdistelmillä.
...
{% include message.html type="info" title="Puuttuva nano" text=note_nano_not_found %}

{% capture note_notepad %}
...
{% include message.html type="info" title="Notepad ja rivinvaihdot" text=note_notepad %}
...
{% include exercise.html points="5%" title="Tehtävä 3: nano" content=exercise_3 %}
...
{% include exercise.html points="5%" title="Tehtävä 4: vim" content=exercise_4 %}
...
{% capture warn_caution_with_root %}
...
{% include message.html type="warning" title="Juurihakemisto" text=warn_caution_with_root %}
...
{% include message.html type="info" title="Suhteellinen sijainti" text=note_relative %}

{% capture note_tab_completion %}
Muista käyttää tab-täydennystä! Kokeile aina ennen kuin kirjoitat kokonaisen kansion tai tiedostonnimen, pystytkö täydentämään nimen suoraan painamalla tab-näppäintä.
{% endcapture %}

{% include message.html type="info" title="Säästä sormiasi" text=note_tab_completion %}
...
{% capture note_touch %}
<p>Voit luoda tekstitiedoston avaamalla sen tekstieditorilla ja tallentamalla, tai komennolla <code>touch</code>.</p>
<p>Tarkista, että olet luonut oikeat kansiot ja tiedostot <code>ls</code>-komennolla.</p>
{% endcapture %}

{% include message.html type="info" title="Tiedoston luominen" text=note_touch %}
{% endcapture %}

{% include exercise.html points="5%" title="Tehtävä 5: Tiedostonhallinnan harjoittelu 1/2" content=exercise_5 %}
...
Tiedostoja voi siirtää käyttäen `mv`-komentoa, joka tulee sanasta <b>m</b>o<b>v</b>e. Komento toimii samalla tavalla kuin `cp`, paitsi että alkuperäinen tiedosto poistetaan, ja jää vain uusi tiedosto. Tämä vastaa graafisen käyttöliittymän "leikkaa"-toimintoa. Komennolla `mv` voi myös uudelleennimetä tiedostoja: jos toisena argumenttina annettu polku viittaa samaan kansioon kuin ensimmäinen polku, kopion nimeäminen ja alkuperäisen tiedoston poistamisen tuloksena on uudelleennimetty tiedosto.
...
<div><img src="/assets/lapio_goal.png" alt="Lopputilanne"/></div>
Muista käyttää tab-täydennystä!
{% endcapture %}

{% include exercise.html
  points="5%"
  title="Tehtävä 6: Tiedostonhallinnan harjoittelu 2/2"
  content=exercise_6 %}

{% capture exercise_7 %}

Tiedostoja voidaan poistaa komennolla <code>rm</code>, eli <b>r</b>e<b>m</b>ove. Käyttäen komentoriviä, poista edellisessä tehtävässä luomasi tiedostot. Selvitä sitten, miten voit poistaa hakemiston komentoriviltä, ja poista koko <code>koulu</code>-hakemisto.

{% capture warn_no_trash %}
Huomaa, että kun poistat jotain komentoriviltä, se ei siirry roskakoriin vaan poistuu heti. Ole siis varovainen.
{% endcapture %}

{% include message.html type="warning" title="Poistaminen todellakin tarkoittaa poistamista" text=warn_no_trash %}
{% endcapture %}

{% include exercise.html points="5%" title="Tehtävä 7: Kansion poistaminen" content=exercise_7 %}
...
{% endcapture %}
{% include exercise.html points="10%" title="Tehtävä 8: Villikortti" content=exercise_8 %}

<h2> Muita hyödyllisiä komentoja</h2>

Joskus on hyödyllistä pystyä tarkastelemaan tekstitiedostoja suoraan komentoriviltä avaamatta erillistä editoria. Helpoin komento tätä varten on `cat` (con<b>cat</b>enate), joka tulostaa suoraan tiedoston sisällön:
...
{% include message.html type="info" text="Vinkki: Kirjainkoko on englanniksi \"case\"." %}
...
{% endcapture %}
{% include exercise.html points="10%" title="Tehtävä 9: grep" content=exercise_9 %}
...
{% include message.html type="info" title="Kokeilemalla opit" text=note_test %}
...
Esimerkissä siis `ls`-komennon tulostus ohjattiin putken avulla edelleen komennolle `grep`. Ilman `|`-operaattoria, ensimmäisen komennon tulostus olisi pitänyt ensin kirjoittaa tiedostoon ja sen jälkeen lukea ja filtteröidä `grep`illä. Komentoa `grep` käyttäen tulosteen pystyi suoraan jakamaan komentojen välillä.
...
{% include message.html type="info" title="Tulosteen ohjaaminen tiedostoon" text=note_shell_redirect %}
...
{% endcapture %}
{% include exercise.html points="5%" title="Tehtävä 10: Opiskelijan muistiinpanot" content=exercise_10 %}

<h2> SSH-yhteys</h2>

Olemme tähän asti käyttäneet komentoriviä lähinnä omalla koneella olevien asioiden tarkasteluun. Komentoriviltä voidaan kuitenkin ottaa myös yhteys toiseen tietokoneeseen. SSH on salattu yhteystyyppi, joka mahdollistaa järjestelmään kirjautumisen etänä komentorivin kautta. Salattu yhteys tarkoittaa, että tietoa ei siirretä suoraan luettavassa muodossa. Tarkoitus on, että vain valikoidut vastaanottajat pystyvät purkamaan salauksen ja lukemaan sisällön.
...
{% capture warn_cs_server_credentials %}
Nykyään näiden palvelimien käyttäminen vaatii erillistä käyttäjätunnusten aktivointia, joka tapahtuu olemalla yhteydessä <a href="https://helpdesk.it.helsinki.fi/">Helpdeskiin</a>. Tällä käytännöllä pyritään minimoimaan vahinkoja tapauksissa, joissa opiskelijan AD-tunnukset ovat joutuneet vääriin käsiin. <b>Huomaa myös, että nämä palvelimet ovat ainoastaan Helsingin Yliopiston tietojenkäsittelytieteen pääaineopiskelijoiden käyttöön.</b>
{% endcapture %}

{% include message.html type="warning" title="Oikeudet Helsingin Yliopiston palvelimille" text=warn_cs_server_credentials %}
...
{% capture note_no_display_pw %}
Huomaa, että salasanaa kirjoittaessasi on normaalia, että komentoriville ei ilmesty tekstiä.
{% endcapture %}

{% include message.html type="info" text=note_no_display_pw %}
...
Edellä yhteys osaston koneeseen autentikoitiin osaston tunnuksilla. SSH-yhteys tarjoaa kuitenkin myös toisen tavan tunnistaa käyttäjä: julkinen ja yksityinen avainpari. Käytännössä avaimilla tarkoitetaan kahta pitkää ja satunnaiselta vaikuttavaa merkkijonoa, joita käytetään yhdessä varmistamaan, että vain halutuilla henkilöillä on pääsy tiettyyn tietoon. Kyseessä on salauksen tekniikka, joka ei kuulu sen enempää tämän kurssin aihealueisiin, mutta voit etsiä asiasta lisää tietoa avainsanoilla _public key encryption_ ja _asymmetric cryptography_. **Riittää ymmärtää, että yksityisen avaimen tulisi olla vain sen omistajan tiedossa, ja julkista avainta voi jakaa huoletta.**

SSH-avainparin voi luoda komennolla `ssh-keygen`. Komento kysyy avaimelle salasanaa ja tallennuspaikkaa. Oletussijaintia tai tiedoston nimeä ei lähtökohtaisesti tarvitse muuttaa, jolloin avain luodaan kotihakemistossa sijaitsevaan `.ssh`-kansioon.

{% capture warn_passphrase %}
Käytä SSH-avainta luodessasi aina salasanaa. Jos käytät SSH-avainta osaston palvelimien kanssa, on salasanan asettaminen pakollista yliopiston määräysten mukaan.
{% endcapture %}

{% include message.html type="warning" title="SSH-avaimen salasana" text=warn_passphrase %}
...
Alla näkyy kuva avainten oikeista sijainneista, kun SSH-avainta käytetään osaston koneille kirjautumiseen.
...
{% include message.html type="info" title="Avaimen tallennus palvelimelle" text=note_copy_key %}
...
{% include message.html type="info" title="Seuraaminen osaston laitteilla" text=note_faculty_devices %}

{% capture note_specify_key %}
...
{% include message.html type="info" title="Avaimen polun määritteleminen" text=note_specify_key %}
{% endcapture %}
{% include exercise.html points="10%" title="Tehtävä 11: SSH-yhteys" content=exercise_11 %}
...
Tarvittavan komennon nimi on `wget` (alunperin _world wide web get_). Se toimii yksinkertaisimmillaan `wget sivun.osoite`, joka lataa annetun sivun koneellesi. Internet-sivut on tehty HTML-kielellä, johon tutustumme seuraavassa osassa, joten saat kutsun tuloksena koneellesi `.html`-päätteisen tiedoston.

{% capture exercise_12 %}

<ol>
<li>Lataa mieleisesi sivusto koneellesi <code>wget</code>-komennolla</li>
<li>Avaa tallennettu <code>.html</code>-päätteinen tiedosto selaimella, esimerkiksi Firefoxilla tai Chromella (saat tiedoston helpoiten selaimella auki graafisesta tiedostonhallinnasta). Sivu näyttää luultavasti hieman erilaiselta kuin yleensä. </li>
...
{% endcapture %}
{% include exercise.html points="5%" title="Tehtävä 12: Wget ja HTML-koodi" content=exercise_12 %}
...
{% endcapture %}
{% include exercise.html points="10%" title="Tehtävä 13: Wget, grep ja Linus Torvalds" content=exercise_13 %}
...
Oikeuksia voi muuttaa tiedoston tai kansion omistaja, tai järjestelmän ylläpitäjä. Niiden muokkaamiseen komentoriviltä tarkoitettu komento on `chmod` (<b>ch</b>ange <b>mod</b>e). Se ottaa argumenttina merkkijonon, joka kertoo lisätäänkö vai poistetaanko oikeuksia, kenelle oikeuksia annetaan, sekä tiedoston tai kansion jolle komento tulee suorittaa.
...
{% capture note_faculty_devices_2 %}
...
{% endcapture %}
{% include message.html type="info" title="Osaston laitteita käytettäessä" text=note_faculty_devices_2 %}
{% endcapture %}

{% include exercise.html points="10%" title="Tehtävä 14: Oikeuksien harjoittelua" content=exercise_14 %}

Voit lukea lisää oikeuksista esimerkiksi [täältä](https://csguide.cs.princeton.edu/account/groups).

{% capture warn_permissive_permissions %}
Ole varovainen oikeuksien kanssa! Liiallisten oikeuksien antaminen esimerkiksi kotihakemistoosi on turvallisuusuhka. Useimmiten tarvitset ajo- ja lukuoikeuksia vain itse. On hyvä käytäntö antaa tiedostoihin vain niin paljon oikeuksia, kuin tiedät olevan välttämätöntä. 
{% endcapture %}

{% include message.html type="warning" title="Vähimpien oikeuksien periaate" text=warn_permissive_permissions %}
...
{% endcapture %}
{% include exercise.html points="5%" title="Tehtävä 15: Pieni bash-skripti" content=exercise_15 %}
...
Normaalikäyttäjä voi kuitenkin ajaa komentoja pääkäyttäjän oikeuksin, jos käyttäjä kuuluu (_sudoers_) ryhmään. Tämä tapahtuu lisäämällä komennon alkuun `sudo` (alunperin _superuser do_). Tällöin ennen komennon ajamista käyttäjältä pyydetään käyttäjän oma salasana. Riippuen järjestelmästä, `sudo`-oikeuksia voidaan tarvita esimerkiksi ohjelmien asentamiseen. Komennon käyttöä tulee kuitenkin välttää, jos mahdollista. Sudon käyttö on joskus tarpeellista, mutta älä esimerkiksi lisää sitä jokaisen komennon alkuun välttääksesi virheviestejä tiedostojen oikeuksista.
...
{% capture warn_sudo_power %}
Älä esimerkiksi kopioi netistä komentoa, joka alkaa sanalla "sudo", jos et ymmärrä mitä se tekee. Annat tällöin ajettaville ohjelmille täydet valtuudet järjestelmässäsi.
{% endcapture %}

{% include message.html type="warning" title="\"Suuren vallan mukana tulee suuri vastuu\"" text=warn_sudo_power %}
...
Ubuntussa ja Cubblissa (osaston koneilla käytettävä Linuxin versio) pitäisi olla apt (tai hyvin samanlainen ohjelma apt-get) valmiina, mutta macOSissa joudut asentamaan homebrew:n erikseen [täältä](https://brew.sh/). Ohjeita aptin käyttöön löytyy [täältä](https://help.ubuntu.com/lts/serverguide/apt.html.en) tai suomeksi [täältä](https://www.linux.fi/wiki/APT). **Suosittelemme vahvasti pakettienhallintajärjestelmän käyttämistä.**
