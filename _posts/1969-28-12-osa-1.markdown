...
<div class="note">
Voit selata <code>man</code>-sivua nuolinäppäimellä, ja postua painamalla <code>q</code>.
</div>
...
Avainparilla kirjautuminen on mahdollista, kun julkinen avain lisätään palvelimelle, jolla avainparilla halutaan autentikoitua. Tällöin kyseiseen järjestelmään pääsee kirjautumaan, jos tietää julkista avainta vastaavan yksityisen avaimen. Lisätään siis `.pub`-päätteisen tiedoston sisältö yliopiston kotihakemiston polkuun `~/.ssh/authorized_keys`, siis kansioon `.ssh`, tiedostoon nimeltä `authorized_keys`. Mikäli käytät useita avaimia, jokainen lisätään omalle rivilleen. Joudut luultavasti luomaan kansion ja tiedoston itse.

Helpoiten saa julkisen avaimen siirrettyä kohdepalvelimelle komennolla <code>scp</code>. Komentoa käytetään seuraavasti:
...
Voit esimerkiksi kopioida julkisen avaimen <code>.ssh</code>-kansioon, ja sen jälkeen uudelleennimetä sen sopivasti.
...
Luo koneellesi SSH-avainpari. Jos olet tietojenkäsittelytieteen läsnäoleva pää- tai sivuaineopiskelija, ja sinulla on tietojenkäsittelytieteen osaston käyttäjätunnus, mahdollista avainparilla kirjautuminen yliopiston palvelimille SSH-yhteyttä luodessa. Testaa sen jälkeen avainparia. Mikäli mahdollista, ota yhteys muulta, kuin osaston koneelta.
