...
2. `em` on verrannollinen sen elementin kirjaisinkokoon, minkä sisällä kyseinen elementti on. Yksi `em` tarkoittaa yhtä kirjaisinkokoa. Taas `2em` tarkoittaa kaksinkertaista kirjaisinkokoa. Esimerkiksi jos ylemmän elementin kirjasinkoko on `12px`, on sisemmän elementin `1em` sama kuin `12px`, ja `2em` sama kuin `24px`.  Kirjaisinkoot, eli `font-size` ominaisuus kannattaa aina merkitä joko pikseleinä tai em-mittayksiköllä.
...
4. `rem` on muuten sama kuin `em`, mutta se on verrannollinen `<html></html>`-elementin kirjaisinkokoon.
...
Tehdään uusi repositorio, minne lisäämme `index.html` tiedoston ja `css` kansioon `styles.css` tiedoston. Tämän jälkeen kirjoitamme `index.html` tiedostoon seuraavan pätkän.
...
Valitaan settings valikosta github pages, ja julkaistaan master branch github sivuna.
